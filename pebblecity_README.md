...
`pebblecity_animated.glb` — an animated, procedurally generated landscape: a
...
| File | Purpose |
|---|---|
| `pebblecity_animated.glb` | the artwork — one self-contained binary |
| `pebblecity_animated.metadata.json` | NFT metadata (edit the CID placeholders) |
...
`tools/generate_pebblecity.py` writes the glTF 2.0 binary directly (Python +
numpy, no Blender in the loop — Blender was not available in this environment).
Regenerate or re-check it with:

```bash
python3 tools/generate_pebblecity.py                    # generate + validate
python3 tools/generate_pebblecity.py --validate-only    # validate only
```
...
|---|---|---|
| `PebbleCity_AllMotion` | 338 | everything (suns + blocks) |
...
- **Drag and drop** onto <https://gltf-viewer.donmccurdy.com> or
  ...
  cameras.
- **`<model-viewer>`**: `<model-viewer src="pebblecity_animated.glb" autoplay
  camera-controls>`.
...
```json
"image":         "ipfs://REPLACE_WITH_YOUR_CID/pebblecity_preview.png",
"animation_url": "ipfs://REPLACE_WITH_YOUR_CID/pebblecity_animated.glb"
```

To mint:

1. Pin `pebblecity_animated.glb` and `pebblecity_preview.png` to IPFS
...
- **Viewers that autoplay only the first clip** will play `PebbleCity_AllMotion`,
...
`pebblecity_loop.gif` samples the animation at 120 evenly spaced points
...
`python3 tools/generate_pebblecity.py --validate-only` re-opens the written
